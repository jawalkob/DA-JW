function patchOut = augmentAndCrop2dPatch(patchIn,outPatchSize,flag)
...
for id=1:size(patchIn,1) 
    ...
    inSize = size(respOut, [1 2]);
    outSize = outPatchSize(1:2);
    win = centerCropWindow2d(inSize, outSize);
    respFinal = imcrop(respOut, win);
    inpVol{id,1} = out;
